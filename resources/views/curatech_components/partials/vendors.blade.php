...
<div class="grid w-full grid-cols-7 grid-flow-cols auto-grid-rows gap-y-2 gap-x-2 text-center max-w-7xl mx-auto h-max px-2">
    ...
    <div class="grid grid-cols-7 col-span-7 text-white">
        ...
        <div class="col-span-2 text-2xl grid auto-rows-max grid-cols-1">
            ...
            @error('form.vendor_product_nr')
                <p class="text-red-500 text-base">{{$message}}</p>
            @enderror
        </div>
        <div class="col-span-2 text-2xl grid auto-rows-max grid-cols-1">
            <x-input-label>Stukprijs</x-input-label>
            @error('form.component_unit_price')
                <p class="text-red-500 text-base">{{$message}}</p>
            @enderror
        </div>
    </div>
    ...
    <form id="add_vendor_form" wire:submit="addVendor" class="grid grid-cols-7 gap-x-2 col-span-7 w-full h-max">
        @csrf
        <x-select-box wire:model="form.vendor_id" name="vendor_id" id="component_add_id_selector" class="col-span-3 text-black">
            <x-slot name="options">
        ...
        <x-text-input wire:model="form.vendor_product_nr" class="col-span-2" type="text" name="vendor_product_nr" id="vendor_product_nr" value="{{old('vendor_product_nr')}}" />
        <x-text-input wire:model="form.component_unit_price" class="col-span-2" type="text" name="component_unit_price" id="component_unit_price" value="{{old('component_unit_price')}}"/>
    </form>
