<div class="grid grid-cols-3 auto-rows-max w-full max-w-7xl bg-gray-300 dark:bg-gray-500 mx-auto py-5 mt-5 rounded text-center">
    ...
    <div class="grid grid-cols-1 auto-rows-max gap-y-4 col-span-2 px-2">
        <p class="text-2xl dark:text-gray-200">Componenten</p>
        ...
        @if(count($comps))
        <ul class="grid grid-cols-3 text-center dark:bg-gray-800 max-h-[400px] overflow-y-scroll">
            @for($i = 0; $i < count($comps); $i++)
                <div class="col-span-3 {{$i % 2 ? 'bg-gray-400' : 'bg-gray-500'}} text-white grid grid-cols-3">
